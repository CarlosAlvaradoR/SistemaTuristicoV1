/**
	MÓDULO DE USUARIOS
*/
SELECT * FROM configuraciones_generales;
...
SELECT p.nombre as nombreP, p.apellidos, p.dni, pt.nombre, 
r.slug, tc.nombre_tipo, 
sc.numero_serie, sp.numero_de_serie, sp.estado,
sp.motivo_de_baja, sp.id as idSeriePago
FROM personas p
INNER JOIN clientes c on p.id = c.persona_id
INNER JOIN reservas r on r.cliente_id = c.id
INNER JOIN paquetes_turisticos pt on pt.id = r.paquete_id
...
-- CREATE OR REPLACE VIEW v_pedidos_informacion_general_pedidos AS
...
;
SELECT nombre_proveedor, ruc, fecha, monto, numero_comprobante, 
ruta_archivo, estado, slug, slugArchivoComprobante, idPedido, pedidos_id,
sum(monto) as montoTotal
FROM v_pedidos_informacion_general_pedidos
GROUP BY pedidos_id;

SELECT SUM(monto) FROM v_pedidos_informacion_general_pedidos
WHERE idPedido = 2;
SELECT * FROM detalle_pedidos;
-- INNER JOIN detalle_pedidos dp on dp.pedidos_id = pe.id;
...
ac.id as idArchivo,ac.ruta_archivo, ac.slug, ac.validez,
cp.pedidos_id
